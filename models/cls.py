...
class Classifier(BaseNet):
    ...
    def __init__(self, feature_dim, input_size, **kwargs):
        super().__init__(**kwargs)

        self.diffusion = kwargs["encoder"]
        self.lstm = nn.LSTM(input_size=input_size, hidden_size=feature_dim, num_layers=2, dropout=0.1,
    ...
    def _calculate_loss(self, batch):
        ...
        if latent_t.dim == 2:
            latent_t = latent_t.unsqueeze(-1)
        out1 = self.diffusion.unet.encode(noisy_img_t, timesteps_t, context=latent_t)
        out1 = self.pool(out1)
        ...
        if latent_t_plus.dim == 2:
            latent_t_plus = latent_t_plus.unsqueeze(-1)
        out2 = self.diffusion.unet.encode(noisy_img_t_plus, timesteps_t_plus, context=latent_t_plus)
        out2 = self.pool(out2)
